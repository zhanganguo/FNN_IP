function cnn = convlifsim_ip(cnn, test_x, test_y, opts)
    ...
    for l = 2 : numel(cnn.layers)   %  for each layer
        if strcmp(cnn.layers{l}.type, 'c')
            ...
            for j = 1 : cnn.layers{l}.outputmaps
                ...
                y = sigma * cnn.layers{l}.spikes{j};
                delta_rC = (1./cnn.layers{l}.rC{j} - y.*I + beta.*(1-y).*I)./tau_ip;
                delta_rR = (-cnn.layers{l}.rR{j} + y - beta.*(1-y))./tau_ip;
